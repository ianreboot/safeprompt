 ...
 *
 * Admin endpoint: Manually block an IP address
 *
 * Body Parameters:
...
import { manuallyBlockIp, isUserAdmin } from '../../../../lib/ip-management.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Block the IP
    const result = await manuallyBlockIp(ip, reason.trim(), keyData.user_id);

    if (!result.success) {
      return res.status(500).json({ error: result.error || 'Failed to block IP' });
    }

    return res.status(200).json({
      success: true,
      message: `IP ${ip} has been manually blocked`,
      reason: reason.trim(),
