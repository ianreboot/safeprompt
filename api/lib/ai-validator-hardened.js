...
const XSS_ATTACK_PATTERNS = [
  ...
  // Event handlers
  /<[^>]+on\w+\s*=\s*[^>]*>/i,  // Any tag with event handler
  ...
  /String\.fromCharCode\s*\(/i,  // CRITICAL: Catch fromCharCode obfuscation
  /eval\s*\(/i,
  /Function\s*\(/i,
  /setTimeout\s*\(/i,
  /setInterval\s*\(/i,
...
const TEMPLATE_INJECTION_PATTERNS = [
  /\{\{[^}]*\}\}/,           // Jinja2/Angular: {{7*7}}
  /\$\{[^}]*\}/,             // JavaScript template literals: ${process.exit()}
  /#\{[^}]*\}/,              // Ruby: #{system('cmd')}
  /<%[^%]*%>/,               // ERB/ASP: <%= system('cmd') %>
  /@\{[^}]*\}/,              // Razor: @{code}
  /\[\[[^\]]*\]\]/,          // MediaWiki/other: [[expression]]
  /\$\([^)]*\)/,             // Shell/Perl: $(command)
];

// SQL injection patterns
const SQL_INJECTION_PATTERNS = [
  /'\s*(OR|AND)\s*['"]?\d+['"]?\s*=\s*['"]?\d+/i,  // ' OR '1'='1, ' OR 1=1
  /'\s*OR\s+\d+\s*=\s*\d+\s*--/i,                   // ' OR 1=1--
  /'\s*;\s*DROP\s+TABLE/i,                          // '; DROP TABLE
  /'\s*;\s*DELETE\s+FROM/i,                         // '; DELETE FROM
  /'\s*;\s*INSERT\s+INTO/i,                         // '; INSERT INTO
  /'\s*;\s*UPDATE\s+\w+\s+SET/i,                    // '; UPDATE users SET
  /UNION\s+SELECT/i,                                 // UNION SELECT
  /'\s*;\s*EXEC\s*\(/i,                             // '; EXEC(
  /'\s*;\s*EXECUTE\s*\(/i,                          // '; EXECUTE(
...
const COMMAND_INJECTION_PATTERNS = [
  /;\s*(ls|cat|rm|wget|curl|nc|bash|sh|python|perl|ruby|php)\s/i,  // Command chaining
  /\|\s*(ls|cat|rm|wget|curl|nc|bash|sh|python|perl|ruby|php)\s/i, // Pipe to command
  /`[^`]*`/,                                        // Backtick execution
...
export async function validateHardened(prompt, options = {}) {
  ...
  const contextForValidators = patternContext.detected ? patternContext : null;

  if (routing.business_validator) {
    validatorPromises.push(
      validateBusiness(prompt, contextForValidators).then(result => ({ type: 'business', result }))
    );
  }

  if (routing.attack_detector) {
    validatorPromises.push(
      detectAttack(prompt, contextForValidators).then(result => ({ type: 'attack', result }))
    );
  }

  if (routing.semantic_analyzer) {
    validatorPromises.push(
      analyzeSemantic(prompt, contextForValidators).then(result => ({ type: 'semantic', result }))
    );
  }
