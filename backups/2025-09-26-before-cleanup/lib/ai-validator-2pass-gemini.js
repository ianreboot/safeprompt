/**
 * SafePrompt 2-Pass Validation System - GEMINI ONLY VERSION
 * Pass 1: Fast Gemini pre-filter with minimal prompt
 * Pass 2: Full Gemini V2 validation when needed
 *
...
// Model configuration - ALL FREE GEMINI
const MODELS = {
  fast: 'google/gemini-2.0-flash-exp:free',
  detailed: 'google/gemini-2.0-flash-exp:free'
};

/**
 * PASS 1: Fast Pre-Filter with Gemini
 * Minimal prompt for quick assessment
 */
...
/**
 * PASS 2: Full V2 Validation with Gemini
...
/**
 * Execute Pass 1: Fast pre-filter with Gemini
 */
async function fastPreFilter(prompt, options = {}) {
  ...
  try {
    ...
    const fullPrompt = FAST_FILTER_PROMPT.replace('{input}', prompt.substring(0, 500)); // Limit input size
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': 'SafePrompt Gemini Fast'
      },
      body: JSON.stringify({
        model: MODELS.fast,
        messages: [
...
/**
 * Execute Pass 2: Full V2 validation with Gemini
 */
  ...
  try {
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'Content-Type': 'application/json',
        'HTTP-Referer': 'https://safeprompt.dev',
        'X-Title': 'SafePrompt Gemini Full'
      },
      body: JSON.stringify({
        model: model,
        messages: [
...
/**
 * Main 2-pass validation function - GEMINI ONLY
 */
export async function validate2PassGemini(prompt, options = {}) {
  const {
    skipPreFilter = false,
    preFilterThreshold = {
  ...
  // PASS 2: Full validation with Gemini for uncertain cases
...
export default { validate2PassGemini, fastPreFilter, fullValidation };