/**
 * Additional Model Combination Tests
 * Testing Llama 8B and Gemini 2.0 paired with Llama 8B for Pass 2
 */
...
const ADDITIONAL_COMBINATIONS = [
  {
    name: 'Llama 8B + Llama 8B',
    pass1: { name: 'meta-llama/llama-3.1-8b-instruct', cost: 0.02 },
    pass2: { name: 'meta-llama/llama-3.1-8b-instruct', cost: 0.02 }
  },
  {
    name: 'Gemini 2.0 + Llama 8B',
...
async function main() {
  console.log('🧪 SafePrompt Additional Model Combination Tests\n');
  console.log('Testing Llama 8B and Gemini 2.0 paired with Llama 8B (Pass 2)');
  console.log('This will take approximately 10 minutes...\n');

  const results = [];

  for (const combination of ADDITIONAL_COMBINATIONS) {
    try {
    ...
    if (ADDITIONAL_COMBINATIONS.indexOf(combination) < ADDITIONAL_COMBINATIONS.length - 1) {
  ...
  console.log('\n\n' + '='.repeat(100));
  console.log('ADDITIONAL MODEL COMBINATION RESULTS');
  ...
  const resultPath = join(__dirname, 'additional-combination-results.json');
